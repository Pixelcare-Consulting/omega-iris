...
export function usePisBySalesCloser(salesCloser: number, dependencies?: any[]) {
  const { execute, executeAsync, isExecuting: isLoading, result } = useAction(getPisBySalesCloserClient)

  useEffect(() => {
    execute({ salesCloser })
  }, [salesCloser, ...(dependencies || [])])
