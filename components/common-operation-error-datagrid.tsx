...
type CommonOperationErrorDataGridProps = {
  title?: string
  description?: string
  isOpen: boolean
  setIsOpen: Dispatch<SetStateAction<boolean>>
  data: CommonOperationError[]
  dataGridRef: Ref<DataGridRef>
  children?: React.ReactNode
  detailsChildren?: React.ReactNode
  detailsColumns?: string[]
  uniqueKey: string
  titleRowAdditionalText?: string
}

export default function CommonOperationErrorDataGrid({
  title,
  description,
  isOpen,
  setIsOpen,
  ...
  detailsColumns,
  uniqueKey,
  titleRowAdditionalText,
}: CommonOperationErrorDataGridProps) {
  const DEFAULT_TITLE = 'Operation Error'
  const DEFAULT_DESCRIPTION = 'There was an error encountered while doing the operation.'
  ...
  const onExporting = (e: DataGridTypes.ExportingEvent) => {
    ...
    exportDataGrid({
      ...
      .then((cellRange: CellRange) => {
        ...
        const columns = [...(detailsColumns ?? []), 'Message']

        columns.forEach((_, cIndex) => {
          if (cIndex === columns.length - 1)
            worksheet.getColumn(detailColStart + cIndex).width = 60 //* 'Message'
          else worksheet.getColumn(detailColStart + cIndex).width = 25 //* other columns
        })
        ...
        for (let i = 0; i < masterRows.length; i++) {
          const id = masterRows[i].rowIndex + i
          const entriesData = masterRows[i].data?.entries || []

          //* Insert title row
          let row = insertRow(id, offset++, 2)
          const columnIndex = cellRangeFromColumn + 1

          Object.assign(row.getCell(columnIndex), {
            value: `# ${masterRows[i].data.id}${titleRowAdditionalText ? ` ${titleRowAdditionalText}` : ''} Errors`,
            fill: titleFill,
          })
          ...
          //* Insert header row
          const columns = [...(detailsColumns ? detailsColumns?.map((c) => titleCase(c)) || [] : []), 'Message']
          row = insertRow(id, offset++, 2)
          ...
          entriesData.forEach((entry: any) => {
            ...
            const detailColumnValues = detailsColumns?.map((col) => entry?.[col] || '') ?? []
            const rowValues = [...detailColumnValues, entry.message]
      ...
      .then(() => {
        workbook.xlsx.writeBuffer().then((buffer) => {
          saveAs(new Blob([buffer], { type: 'application/octet-stream' }), `${uniqueKey}-ERROR-${format(new Date(), 'MM-dd-yyyy')}.xlsx`)
        })
      })
  }
    ...
    <Popup visible={isOpen} dragEnabled={false} maxHeight={700} maxWidth={700} onHiding={() => setIsOpen(false)}>
      <div className='h-full w-full'>
        <DataGrid
          ...
          columnMinWidth={DEFAULT_COLUMN_MIN_WIDTH}
        >
          <Column dataField='id' dataType='number' caption='#' alignment='center' />
          {children}
