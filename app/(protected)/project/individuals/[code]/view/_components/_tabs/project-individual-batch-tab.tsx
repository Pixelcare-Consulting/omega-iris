...
import { getBatchesMasterByProjectCode } from '@/actions/batches'
...
type ProjectIndividualBatchTabProps = {
  projectCode: number
  batches: ReturnType<typeof useBatchesMasterByWarehouseProjectCode>
}

type DataSource = Awaited<ReturnType<typeof getBatchesMasterByProjectCode>>

export default function ProjectIndividualBatchTab({ projectCode, batches }: ProjectIndividualBatchTabProps) {
  const { data: session } = useSession()

  const DATAGRID_STORAGE_KEY = `dx-datagrid-batch-by-project-${projectCode}`
  const DATAGRID_UNIQUE_KEY = `project-individual-batches-${projectCode}`
  ...
  return (
    <div className='flex h-full w-full flex-col'>
      {!isViewMode ? (
        ...
        <ProjectIndividualBatchView data={rowData} onClose={handleClose} />
      )}
    </div>
  )
}
