...
import { CustomRule, DataGridRef, DataGridTypes, Editing, GroupItem, Summary, TotalItem } from 'devextreme-react/data-grid'
...
export default function WorkOrderLineItemForm({
  ...
  const mainFormLineItems = useWatch({ control: mainForm.control, name: 'lineItems' }) || []
  ...
  }, [JSON.stringify(lineItems), JSON.stringify(mainFormLineItems)])
  ...
  const handleOnRowUpdated = useCallback(
    ...
    [JSON.stringify(projectItemsDataSource), JSON.stringify(mainFormLineItems), JSON.stringify(lineItems)]
  )
